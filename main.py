...
def check_password_complexity(password):
    try:
        if len(password) >= 12 and re.search(r'[A-Z]', password) and re.search(r'[a-z]', password) and re.search(
                r'[0-9]', password):
            return True
        else:
            return False
...
def createEstate(account):
    try:
        ...
        type = int(
            input("Выберите тип недвижимости:\n1. Дом\n2. Квартира\n3. Промышленный объект\n4. Дача\nВыш выбор: "))
        if type < 1 or type > 4:
        ...
        if len(estates) > 0:
            for estate in estates:
                ...
                if estate[4] == True:
                    ...
                    status = "Неактивный"
                print(
                    f"Адрес: {estate[0]}, номер: {estate[1]}, тип: {estate_type}, создатель: {estate[3]}, статус: {status}, ID: {estate[5]}")
        else:
        ...
        if len(myestates) > 0:
            for estate in myestates:
                ...
                if estate[4] == True:
                    ...
                    status = "Неактивный"
                print(
                    f"Адрес: {estate[0]}, номер: {estate[1]}, тип: {type}, создатель: {estate[3]}, статус: {status}, ID: {estate[5]}")
        ...
        if len(ads) > 0:
            for ad in ads:
                ...
                if ad[5] == 0:
                    ...
                    status = "Закрыт"
            print(
                f"ID объявления: {ad[0]}, цена: {ad[1]}, ID недвижимости: {ad[2]}, создатель: {ad[3]}, покупатель: {buyer}, дата: {ad[5]}, статус: {status}")
        else:
...
def give_balance_to_user(public_key):
    try:
        w3.eth.send_transaction({'to': public_key, 'from': "0x9129A96678342Ea12e2e78189a81971e28468576", 'value': w3.to_wei(10, 'ether')})
...
def deposit_to_contract(account):
